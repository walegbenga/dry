...
jQuery(function($){
    //console.log("init.js was just loaded successfully");
...
$("body").on("click", ".admin-options,.new", function(event){

    // Prevents the form from submitting
    event.preventDefault();

    // Sets the action for the form submission
    var action = $(event.target).attr("name") || "new",

    // Saves the value of the event_id input
        id = $(event.target)
                .siblings("input[name=id]")
                    .val();
...
$("body").on("click", ".theForm input[type=submit]", function(event){
    ...
    /*if ( $(this).attr("name")=="event_submit" && $(".active").length > 0 )
    {
        ...
        var titleArray = $.grep(formArray, function(elem) {
            return elem.name === 'add a new parastatal';
        });
